...
static Moon moon;
// An orbiter is an object that flies on a circle of a certain radius on the xz plane. You supply the radius at construction time.
class Orbiter {
	double radius;
	double u;
public:
...
// Clears the window (and the depth buffer) and draws the moon as viewed from the current position of the orbiter.
void display() {
...
// reshape() fixes up the projection matrix so that we always see a sphere instead of an ellipsoid.
...
// Enables depth testing, enables lighting for a bright yellowish diffuse light, and creates a moon.
void init() {
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("Mahmoud Alyosify Moon");
